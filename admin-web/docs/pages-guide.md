# 管理端页面使用指南

## 📋 已实现功能

### 1. 用户管理 (`/system/user`)
- ✅ 用户列表展示
- ✅ 搜索(用户名/真实姓名)
- ✅ 新增用户(Drawer 表单)
- ✅ 编辑用户(Drawer 表单)
- ✅ 禁用/启用用户
- ✅ 重置密码
- ✅ 分页功能

### 2. 角色管理 (`/system/role`)
- ✅ 角色列表展示
- ✅ 新增角色(Drawer 表单)
- ✅ 编辑角色(Drawer 表单)
- ✅ 删除角色
- ✅ 绑定权限点(Tree 选择)
- ✅ 绑定菜单(Tree 选择)

### 3. 权限点管理 (`/system/permission`)
- ✅ 权限列表展示
- ✅ 新增权限(Drawer 表单)
- ✅ 编辑权限(Drawer 表单)
- ✅ 删除权限
- ✅ 权限类型(MENU/BUTTON/API)

### 4. 菜单管理 (`/system/menu`)
- ✅ 菜单树表格展示
- ✅ 新增菜单(Drawer 表单)
- ✅ 新增子菜单
- ✅ 编辑菜单(Drawer 表单)
- ✅ 删除菜单(级联删除)
- ✅ 排序设置
- ✅ 显示/隐藏切换

---

## 🚀 快速开始

### 前提条件

1. **后端服务已启动**
2. **已初始化数据**:
```bash
cd server
python scripts/seed_admin.py
python scripts/seed_permissions.py
python scripts/seed_menus.py
```

### 启动前端

```bash
cd admin-web
npm install
npm run dev
```

访问: http://localhost:5173

---

## ✅ 功能测试

### 1. 用户管理测试

#### 1.1 查看用户列表
1. 登录系统(admin/admin123)
2. 点击左侧菜单"系统管理" → "用户管理"
3. 查看用户列表

#### 1.2 新增用户
1. 点击"新增用户"按钮
2. 填写表单:
   - 用户名: test_user
   - 真实姓名: 测试用户
   - 手机号: 13800138000
   - 邮箱: test@example.com
   - 密码: 123456
3. 点击"确定"
4. 验证用户创建成功

#### 1.3 编辑用户
1. 点击用户列表中的"编辑"按钮
2. 修改用户信息
3. 点击"确定"
4. 验证更新成功

#### 1.4 重置密码
1. 点击"重置密码"按钮
2. 输入新密码
3. 验证密码重置成功

#### 1.5 禁用/启用用户
1. 点击"禁用"按钮
2. 确认操作
3. 验证用户状态变更

### 2. 角色管理测试

#### 2.1 新增角色
1. 进入"角色管理"页面
2. 点击"新增角色"
3. 填写表单:
   - 角色名称: 测试角色
   - 角色编码: TEST_ROLE
   - 描述: 测试角色描述
4. 点击"确定"

#### 2.2 绑定权限
1. 点击"绑定权限"按钮
2. 在权限树中勾选权限
3. 点击"确定"
4. 验证权限绑定成功

#### 2.3 绑定菜单
1. 点击"绑定菜单"按钮
2. 在菜单树中勾选菜单
3. 点击"确定"
4. 验证菜单绑定成功

### 3. 权限点管理测试

#### 3.1 新增权限
1. 进入"权限管理"页面
2. 点击"新增权限"
3. 填写表单:
   - 权限名称: 测试权限
   - 权限编码: sys:test:view
   - 类型: API
   - 描述: 测试权限描述
4. 点击"确定"

### 4. 菜单管理测试

#### 4.1 新增菜单
1. 进入"菜单管理"页面
2. 点击"新增菜单"
3. 填写表单:
   - 菜单标题: 测试菜单
   - 路由名称: TestMenu
   - 路由路径: /test
   - 组件路径: views/test/index.vue
   - 图标: Document
   - 排序: 10
4. 点击"确定"

#### 4.2 新增子菜单
1. 点击某个菜单的"新增子菜单"按钮
2. 填写表单
3. 验证子菜单创建成功

---

## 🎨 页面设计说明

### 统一风格

- **Element Plus**: 所有组件使用 Element Plus
- **Drawer 表单**: 所有新增/编辑使用 Drawer
- **颜色规范**:
  - 主色: #409EFF(蓝色)
  - 成功: #67C23A(绿色)
  - 警告: #E6A23C(橙色)
  - 危险: #F56C6C(红色)

### 布局结构

```
┌─────────────────────────────────────┐
│  搜索栏 + 操作按钮                    │
├─────────────────────────────────────┤
│                                     │
│  数据表格                            │
│                                     │
│  分页                                │
└─────────────────────────────────────┘
```

### Drawer 表单

- 宽度: 600px
- 位置: 右侧
- 底部: 取消 + 确定按钮
- 验证: 必填项标红

---

## 🔧 路由与菜单配置

### 路由配置方式

**后端返回菜单格式**:
```json
{
  "id": 10,
  "title": "用户管理",
  "name": "User",
  "path": "/system/user",
  "component": "views/system/user/index.vue",
  "meta": {
    "icon": "User",
    "title": "用户管理",
    "hidden": false,
    "keepAlive": true
  }
}
```

**前端动态加载**:
```javascript
// router/index.js 中的 menuToRoutes 函数
function menuToRoutes(menus) {
  return menus.map(menu => ({
    path: menu.path,
    name: menu.name,
    component: () => import(`@/views/${menu.component}`),
    meta: menu.meta,
    children: menu.children?.map(menuToRoutes)
  }))
}
```

### 组件路径约定

| 菜单 | 组件路径 |
|---|---|
| 用户管理 | `views/system/user/index.vue` |
| 角色管理 | `views/system/role/index.vue` |
| 权限管理 | `views/system/permission/index.vue` |
| 菜单管理 | `views/system/menu/index.vue` |

---

## 📊 API 接口说明

### 用户管理 API

- `GET /api/v1/admin/users` - 获取用户列表
- `POST /api/v1/admin/users` - 创建用户
- `PUT /api/v1/admin/users/{id}` - 更新用户
- `DELETE /api/v1/admin/users/{id}` - 删除用户
- `POST /api/v1/admin/users/{id}/reset-password` - 重置密码

### 角色管理 API

- `GET /api/v1/admin/roles` - 获取角色列表
- `POST /api/v1/admin/roles` - 创建角色
- `PUT /api/v1/admin/roles/{id}` - 更新角色
- `DELETE /api/v1/admin/roles/{id}` - 删除角色
- `POST /api/v1/admin/roles/{id}/permissions` - 绑定权限
- `POST /api/v1/admin/roles/{id}/menus` - 绑定菜单

### 权限管理 API

- `GET /api/v1/admin/permissions` - 获取权限列表
- `POST /api/v1/admin/permissions` - 创建权限
- `PUT /api/v1/admin/permissions/{id}` - 更新权限
- `DELETE /api/v1/admin/permissions/{id}` - 删除权限

### 菜单管理 API

- `GET /api/v1/admin/menus/tree` - 获取菜单树
- `POST /api/v1/admin/menus` - 创建菜单
- `PUT /api/v1/admin/menus/{id}` - 更新菜单
- `DELETE /api/v1/admin/menus/{id}` - 删除菜单

---

## ⚠️ 注意事项

1. **权限控制**: 所有操作按钮都使用 `v-perm` 指令控制显示
2. **数据验证**: 表单提交前进行验证
3. **错误处理**: 所有 API 调用都有错误处理
4. **用户反馈**: 操作成功/失败都有提示信息
5. **确认操作**: 删除等危险操作需要二次确认

---

## 🔄 下一步

1. ✅ 实现后端对应的 API 接口
2. ✅ 测试所有页面功能
3. ✅ 优化用户体验
4. ✅ 添加更多业务页面
